...
async def hybrid_search(q: str = Query(..., description="Search query"), top_k: int = 5):
    try:
        # Dense metadata
        dense_q = encode_texts([q], normalize=True)[0].astype('float32').reshape(1, -1)
        distances_dense, indices_dense = await run_in_threadpool(dense_index.search, dense_q, top_k)
        scores_dense = -distances_dense[0]

        # SPLADE sparse
        sparse_q = list(splade_model.embed([q]))[0]
        ...
        distances_image, indices_image = await run_in_threadpool(image_index.search, query_image_embedding, top_k)
        ...
        for i in range(top_k):
            idx = indices_dense[0][i]  # Based on dense index ordering
